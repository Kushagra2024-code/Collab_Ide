...
- `emitToProject(projectId, event, data)` and `emitToUser(userId, event, data)` are exported from `socket.ts` for use in route handlers. `_io` singleton is set by `initSocket`; no circular import risk since routes never import from routes.
- Cursor moves are throttled to 50ms per (socketId, fileId) pair — do not remove this throttle.

## React Query cache keys
- Always use the generated key getters (e.g. `getListFilesQueryKey(projectId)`, `getGetProjectQueryKey(projectId)`) when doing `queryClient.setQueryData` or `invalidateQueries`. Using ad-hoc arrays like `['files', id]` hits a different cache entry and the update is silently ignored.
...
- Express 5 handles async route errors natively — no need for an `asyncHandler` wrapper.
- `setActiveFileId` inside a `setOpenFiles` functional updater is the correct pattern when the new active tab depends on the post-deletion `openFiles` state.
